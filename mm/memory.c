...
static bool tfork_one_pte_table(struct mm_struct *, pmd_t *, unsigned long,
				unsigned long);
			   ...
			   unsigned long addr)
{
	...
	pmd_clear(pmd);
	// if refcount > 0, the pte table is still in use
	if (atomic64_read(&token->pte_table_refcount) > 0)
		return;
 ...
 */
int dereference_pte_table(pmd_t pmd_val, bool free_table, struct mm_struct *mm,
		unsigned long addr)
{
...
int dereference_pte_table_multiple(pmd_t pmd_val, bool free_table,
	       	struct mm_struct *mm, unsigned long addr, int num)
{
	struct page *table_page;
	int count_after;
		   ...
		   unsigned long addr, unsigned long end)
{
	...
	//src_pte points to the old table
	src_pte = tfork_pte_offset_kernel(src_pmd_val, addr);
	if (!pmd_iswrite(*dst_pmd)) {
		//dst_pte points to a new table
	...
	//kyz: associates the VMA with the new table
	atomic64_inc(&(dst_pte_page->pte_table_refcount));
	       ...
	       unsigned long end)
{
	...
	do {
		...
		if (src_mm->flags & MMF_USE_ODF_MASK) {
			...
			/* kyz: sets write-protect to the pmd entry
			 * if the vma is writable
			 */
			...
			/* kyz : the old VMA hasn't been counted in
			 * the PTE table, count it now
			 */
			if (src_vma->pte_table_counter_pending) {
				atomic64_add(2,
					&(table_page->pte_table_refcount));
			} else {
				//increments the pte table counter
				atomic64_inc(&(table_page->pte_table_refcount));
				/* the VMA is old, but the pte table is new
				 * (created by a fault after the last odf call)
				 */
				if (atomic64_read(
				      &(table_page->pte_table_refcount)) == 1) {
					atomic64_set(
					  &(table_page->pte_table_refcount), 2);
				}
			}
			// shares the table with child
			set_pmd_at(dst_mm, addr, dst_pmd, src_pmd_value);
		} else {
				...
				struct zap_details *details)
{
	...
	do {
		...
		if (pmd_none_or_trans_huge_or_clear_bad(pmd))
			goto next;
		/* kyz: copy if the pte table is shared and VMA does not cover
		 * fully the 2MB region
		 */
		...
		if ((!pmd_iswrite(*pmd)) && (!vma->pte_table_counter_pending)) {
			...
			if (table_start < vma->vm_start || table_end > vma->vm_end) {
				/* dec the counter of the shared table.
				 * tfork_one_pte_table() cannot find the
				 * current VMA (which is being unmapped)
				 */
				if (dereference_pte_table(*pmd, false,
						vma->vm_mm, addr) != 1) {
					got_new_table =
					       tfork_one_pte_table(vma->vm_mm,
								   pmd, addr,
								   vma->vm_end);
					if (got_new_table) {
				...
				/* kyz: shared and fully covered by the VMA,
				 * preserve the pte entries
				 */
				next = zap_pte_range(tlb, vma, pmd,
						addr, next, details, true);
				dereference_pte_table(*pmd, true,
						vma->vm_mm, addr);
				pmd_clear(pmd);
			}
		} else {
			next = zap_pte_range(tlb, vma, pmd,
					addr, next, details, false);
...
static bool tfork_one_pte_table(struct mm_struct *mm, pmd_t *dst_pmd, unsigned long addr, unsigned long exclude)
{
	...
	do {
		...
		/* this vma is newly mapped (clean) and (fully/partly)
		 * described by this pte table
		 */
		if (vma->pte_table_counter_pending) {
			addr = end;
			continue;
	...
	dereference_pte_table_multiple(orig_pmd_val, true, mm,
					orig_addr, num_vmas);
	return copied;
}
